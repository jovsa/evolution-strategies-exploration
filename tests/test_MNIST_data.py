...
def test_data_size_test():
    assert(np.shape(mnist.train.images)[0] == np.shape(mnist.train.labels)[0]==55000)

def test_data_size_train():
    assert(np.shape(mnist.test.images)[0] == np.shape(mnist.test.labels)[0]== 10000)

def test_data_size_validation():
    assert(np.shape(mnist.validation.images)[0] == np.shape(mnist.validation.labels)[0]== 5000)
